    ...
    <div className="page-inner">
      ...
      {scanPopup && scanPopup.success && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'white', borderRadius: 28, padding: '36px 32px', textAlign: 'center', maxWidth: 380, width: '100%', boxShadow: '0 32px 80px rgba(0,0,0,0.3)', position: 'relative' }}>
            {/* Close button */}
            <button
              onClick={() => setScanPopup(null)}
              style={{ position: 'absolute', top: 16, right: 16, width: 36, height: 36, borderRadius: '50%', border: 'none', background: '#F1F5F9', cursor: 'pointer', fontSize: 16, color: '#64748B', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, zIndex: 1 }}
            >✕</button>

            {/* Status badge */}
            <div style={{ display: 'inline-block', padding: '4px 14px', borderRadius: 20, marginBottom: 20,
            ...
            {/* Photo — big, Cloudinary URL preferred, legacy base64 as fallback */}
            <div style={{
              width: 220, height: 220, borderRadius: '50%', margin: '0 auto 20px',
              padding: 6, background: 'white',
              boxShadow: `0 0 0 4px ${scanPopup.status === 'PRESENT' ? '#86EFAC' : scanPopup.status === 'LATE' ? '#FDE047' : '#FCA5A5'}, 0 12px 32px rgba(0,0,0,0.18)`,
            }}>
              <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden', background: '#F1F5F9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {scanPopup.photoUrl ? (
            ...
            {/* Name */}
            <h2 style={{ fontFamily: 'Nunito', fontSize: 24, fontWeight: 900, color: '#1E3A5F', margin: '0 0 10px' }}>
            ...
            {scanPopup.grade && (
              <div style={{
                ...
                background: '#E1F5FE', color: '#0288D1', fontFamily: 'Nunito', fontWeight: 800, fontSize: 14,
                padding: '6px 18px', borderRadius: 20, marginBottom: 14,
              }}>
                🎓 Grade {scanPopup.grade}
              </div>
            )}
